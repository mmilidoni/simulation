...
//    private int clock;
    ...
    private long tempiRisposta[][];
    private boolean stop = false;
    private FramePlot frame;

    public Main1(double gamma, double mu, int p, int n0, FramePlot frame) {
...
//      clock = 0;
    ...
    public static void main(String[] args) {
        ...
        for (int n = 1; n <= nn0; n++) {
            Main1 m = new Main1(0.5, 0.5, pp, n, frame);
            m.sequenziatore();
        }
    }
    ...
    private void sequenziatore() {
        while (!stop) {
            if (nRun == p) {
                calendario.setSimulazione(new Evento(0d, TipoEvento.FINE_SIMULAZIONE));
            }
            calendario.setArrivo(new Evento(genExp.next(), TipoEvento.ARRIVO));
    ...
    private void arrivo() {
        ...
        if (jobCorrenteCpu == null) {
            jobCorrenteCpu = job;
            calendario.setCpu(new Evento(job.getCarico(), TipoEvento.FINE_CPU));
    ...
    private void fineCPU() {
        ...
        if (routing <= 0.9) {
            if (jobCorrenteIO == null) {
                ...
                calendario.setIo(new Evento(jobCorrenteIO.getCarico(), TipoEvento.FINE_IO));
            ...
            tempiRisposta[nRun][nOss++] = temp.getTempoRisposta();
        ...
        if (!cpuQueue.isEmpty()) {
            ...
            calendario.setCpu(new Evento(jobCorrenteCpu.getCarico(), TipoEvento.FINE_CPU));
